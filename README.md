...
**Core**: `dot`, `cosine`, `norm`, `l2_distance`, `l2_distance_squared`, `l1_distance`, `angular_distance`. Each has a `_portable` fallback for non-SIMD targets.
...
**Scalar quantization (uint8)**: `scalar::QuantizationParams` (from `fit()` or `from_range()`), `quantize_u8`, `asymmetric_dot_u8`. 4x compression.

**Fast approximate math**: `fast_cosine`, `fast_rsqrt` (hardware rsqrt + Newton-Raphson).

**Batch operations**: `batch::VerticalBatch` (PDX-style columnar layout) with `batch_dot`, `batch_l2_squared`, `batch_cosine`, `batch_knn`, `batch_knn_cosine`, `batch_knn_dot`, `batch_knn_filtered` (predicate pushdown), `batch_knn_reordered` (variance-ordered pruning), `batch_knn_adaptive` (approximate early-exit).

**Sparse vectors**: `sparse_dot`, `sparse_maxsim`.

**ColBERT late interaction**: `maxsim`, `maxsim_cosine`.
